...
export class GhostCursor {
    ...
    private ghostTrails: NodeJS.Timeout[] = [];
    ...
    private createGhostCursorWebview() {
    ...
    private createGhostTrail(position: vscode.Position) {
        if (!this.isEnabled) return;
    ...
    private showGhostAtPosition(position: vscode.Position, index: number) {
        if (!this.isEnabled) return;
    ...
    private clearGhostTrails() {
        this.ghostTrails.forEach(timeout => clearTimeout(timeout));
        this.ghostTrails = [];
    }

    private getGhostCursorHtml(): string {
        return `
            <!DOCTYPE html>
            <html>
            <head>
